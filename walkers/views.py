...
from orderForm.models import walkersOrder, WalkersOrderProduct
from walkers.models import walkersProduct, Category

def walkers(request):
    products = walkersProduct.objects.all()
    ...
    for product in walkersProduct.objects.all():
    ...
    if request.method == 'POST':
        order = walkersOrder.objects.create()
        for product in walkersProduct.objects.all():
            ...
            if product_id_field in request.POST and quantity_field in request.POST:
                try:
                    ...
                    if quantity > 0:
                        product_instance = walkersProduct.objects.get(pk=request.POST[product_id_field])
                        WalkersOrderProduct.objects.create(order=order, product=product_instance, quantity=quantity)
                except ValueError:
                    continue
    ...
    return render(request, 'walkers/walkers-order.html', context)
